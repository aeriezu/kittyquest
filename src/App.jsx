...
import { C, px, EMPTY_EQ, EMPTY_HOUSE, SHOP, QUEST_TEMPLATES, SUBJECT_PALETTE, PET_OPTIONS } from "./data/constants";
import { useGameState, getXpLevel } from "./hooks/useGameState";
...
// ─── Import Schedule ──────────────────────────────────────────────────────────
function ImportSchedule({ onImport }) {
...
function TasksTab({ days, subjects, checked, onToggle, onAddTask, onDeleteTask, onDeleteDay, onImport, onEditSubjects, petInfo, onEditTask, onMoveTask }) {
  ...
  return (
    <div>
      ...
      <ImportSchedule onImport={onImport} />
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.6)", display:"flex", alignItems:"flex-end", justifyContent:"center", zIndex:1000 }}>
      <div style={{ background:C.bg, borderRadius:"16px 16px 0 0", padding:16, width:"100%", maxWidth:480, maxHeight:"80vh", overflowY:"auto", ...px }}>
        ...
        <div style={{ marginBottom:12 }}><HouseRoom house={local} compact={true} /></div>
        ...
        <div style={{ display:"flex", gap:4, marginBottom:10 }}>
          {["wallpaper","floor","furniture"].map(s => (
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:5, marginBottom:12 }}>
          {sectionItems.map(item => {
            ...
            const isActive = section==="furniture" ? [local.left,local.center,local.right].includes(item.id) : local[item.slot]===item.id;
            return (
              <div key={item.id} style={{ background:isOwned?(isActive?"#d4f0d4":C.surface):item.rare?"#f5f0ff":C.surface, borderRadius:10, padding:"9px 10px", border:`1px solid ${isOwned?(isActive?C.green:C.surface2):item.rare?C.purple:C.surface2}`, display:"flex", flexDirection:"column", gap:3 }}>
                ...
                {isOwned&&<button onClick={() => equip(item)} style={{ padding:"3px 0", borderRadius:6, border:"none", fontSize:"0.65rem", fontWeight:700, background:isActive?C.primary:C.green, color:"#fff", cursor:"pointer", fontFamily:"inherit" }}>{isActive?"✓ Placed":"Place"}</button>}
        ...
        <button onClick={() => onSave(local)} style={{ width:"100%", padding:"10px 0", borderRadius:10, border:"none", background:C.primary, color:"#fff", fontFamily:"inherit", fontSize:"0.8rem", fontWeight:700, cursor:"pointer" }}>💾 Save Room</button>
      </div>
    </div>
  );
}
...
// ─── PetTab ───────────────────────────────────────────────────────────────────
function PetTab({ petId, petName, mood, happiness, level, title, totalXp, equipped, owned, onEquip, onChangeCat, house, onEditRoom }) {
  return (
    <div>
      ...
      <div style={{ textAlign:"center", marginBottom:8 }}>
        <div style={{ fontSize:"0.9rem", fontWeight:700, color:C.text, marginBottom:2 }}>{petName}</div>
        <div style={{ fontSize:"0.72rem", color:C.primary, fontWeight:700, marginBottom:4 }}>Lv.{level} {title}</div>
        <div style={{ fontSize:"0.6rem", color:C.muted, marginBottom:6 }}>{totalXp} XP total</div>
...
// ─── QuestsTab ────────────────────────────────────────────────────────────────
function QuestsTab({ coins, achievements, onSpin, dailyQuests, questDone, onComplete, totalTasksDone }) {
  return (
    <div>
          ...
          return (
            <div key={q.id} style={{ background:isDone?"#d4edd4":C.bg, borderRadius:8, padding:"8px 10px", marginBottom:6, border:`1px solid ${isDone?C.green:C.surface2}`, display:"flex", alignItems:"center", gap:8 }}>
              <div style={{ flex:1 }}>
                ...
                <div style={{ fontSize:"0.62rem", color:C.muted }}>+{q.reward} coins · +25 XP</div>
              </div>
              {isDone
                ? <span>✅</span>
                : <button onClick={() => canComplete && onComplete(q)} style={{ padding:"4px 10px", borderRadius:6, border:"none", fontSize:"0.65rem", fontWeight:700, background:canComplete?C.green:C.surface2, color:canComplete?"#fff":C.muted, cursor:canComplete?"pointer":"default", fontFamily:"inherit" }}>
                    {canComplete ? "Claim!" : `+${q.reward}`}
                  </button>
              }
            </div>
          );
...
// ─── ShopTab ──────────────────────────────────────────────────────────────────
function ShopTab({ coins, owned, equipped, onBuy, onEquip, house }) {
  return (
    <div>
...
export default function App() {
  ...
  const allTasks = days.flatMap(d => d.tasks || []).filter(Boolean);
  const { state, done, toggleTask, handleSpin, buyItem, equipItem, cashOut, tickHappiness, incrementCheers, completeQuest, setState } = useGameState(allTasks.length);
  ...
  // ── Derived display ───────────────────────────────────────────────────────
  const { level, title, progress } = getXpLevel(state.totalXp || 0);
  const mood = state.happiness < 30 ? "sleepy" : level >= 9 ? "excited" : level >= 4 ? "neutral" : "sleepy";
  ...
  return (
    <div style={{ ...px, background:C.bg, minHeight:"100vh", padding:16, maxWidth:480, margin:"0 auto" }}>
      ...
      {levelUp && (
        <div style={{ position:"fixed", top:160, left:"50%", transform:"translateX(-50%)", background:"#fff", borderRadius:16, padding:"12px 20px", boxShadow:"0 4px 20px rgba(0,0,0,0.2)", zIndex:1000, textAlign:"center", border:`2px solid ${C.primary}` }}>
          <div style={{ fontSize:"1.5rem" }}>⬆️</div>
          <div style={{ fontSize:"0.8rem", fontWeight:700, color:C.primary }}>Level Up! Lv.{levelUp.level}</div>
          <div style={{ fontSize:"0.72rem", color:C.muted }}>{levelUp.title}</div>
        </div>
      )}
      ...
      {showRoomEditor && <RoomEditor house={house} owned={state.owned} coins={state.coins} onSave={handleSaveRoom} onClose={() => setShowRoomEditor(false)} onBuy={item => buyItem(item, showAchievement)} />}
      ...
      <div style={{ background:C.surface, borderRadius:4, height:7, marginBottom:4, overflow:"hidden", border:`1px solid ${C.surface2}` }}>
        <div style={{ background:C.primary, width:`${Math.min(100,Math.max(0,progress))}%`, height:"100%", transition:"width 0.4s" }} />
      </div>
      <div style={{ display:"flex", justifyContent:"space-between", fontSize:"0.6rem", color:C.muted, marginBottom:10 }}>
        <span>Lv.{level}</span>
        <span>{state.totalXp || 0} XP</span>
      ...
      {tab==="quests" && (
        <QuestsTab coins={state.coins} achievements={achDisplay}
          onSpin={() => setShowSpin(true)} dailyQuests={dailyQuests}
          questDone={state.questDone}
          onComplete={quest => completeQuest(quest, showPopup, showAchievement)}
          totalTasksDone={actualDone} />
      )}
      {tab==="friend" && (
